...
const SurveyBuilder: React.FC<SurveyBuilderProps> = ({
  ...
  // Initialize with proper Survey structure
  ...
  const currentPageIndex = 0; // For now, we only support single page surveys

  // Helper to get current page
  const currentPage = currentSurvey.pages?.[currentPageIndex];
  ...
  }, [currentPage, allQuestions.length, currentPageIndex]);
  ...
  const duplicateQuestion = useCallback((questionId: string) => {
    const questionToDuplicate = allQuestions.find(q => q.id === questionId);
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center space-x-2 ml-4">
            ...
            <button
              ...
              disabled={question.order === allQuestions.length - 1}
              className="text-gray-400 hover:text-gray-600 disabled:opacity-50"
            >
              <ChevronDown size={16} />
            </button>
              ...
              <div className="p-6">
                ...
                <div className="space-y-4">
                  {allQuestions.length === 0 ? (
                    ...
                    allQuestions.map((question) => renderQuestionPreview(question))
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="p-6 max-w-2xl">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    <label className="flex items-center">
                      <input
                        type="checkbox"
                        checked={currentSurvey.settings?.progressBar}
                        onChange={(e) => setCurrentSurvey(prev => ({
                          ...prev,
                ...
                <div>
                  ...
                  <textarea
                    value={currentSurvey.thankYouMessage || ''}
                    onChange={(e) => setCurrentSurvey(prev => ({
                      ...prev,
            ...
            <div className="p-6">
              ...
              <div className="max-w-2xl mx-auto">
                <div className="bg-white border border-gray-200 rounded-lg p-8">
                  ...
                  {currentSurvey.settings?.progressBar && (
                    <div className="mb-8">
                      <div className="flex justify-between text-sm text-gray-600 mb-2">
                        <span>Progress</span>
                        <span>1 of {allQuestions.length || 0}</span>
                      </div>
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <div 
                          className="bg-blue-600 h-2 rounded-full transition-all duration-300"
                          style={{ 
                            width: `${allQuestions.length ? (1 / allQuestions.length) * 100 : 0}%` 
                          }}
                        />
                      </div>
                    </div>
                  ...
                  <div className="space-y-8">
                    {allQuestions.length === 0 ? (
                      ...
                      allQuestions.slice(0, 1).map((question, index) => (
                  ...
                  {allQuestions.length > 0 && (
                    <div className="mt-8 flex justify-between">
                      ...
                      <button className="px-6 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">
                        {allQuestions.length === 1 ? 'Submit' : 'Next'}
                      </button>
                    </div>
                  )}
                </div>
